...
{
    public class ReportedStolen : IDocumentGenerator
    {
         IDocumentGeneratorService _documentGeneratorService=null;

         public ReportedStolen(IDocumentGeneratorService documentGeneratorService)
        {
         ...
         public async Task<byte[]> GetBytesAsync(DocumentGeneratorViewModel request)
         {
             ...
             if (request != null)
             {
                 ...
                 string template = template = RazorHelper.ParseTemplate(DocumemtTemplateUrl.Reported_Stolen, model);
